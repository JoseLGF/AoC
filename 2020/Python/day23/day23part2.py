...
def puzzleSolution1(lines):
    ...
    num1index = cups.index(1)
    # return the two numbers after that
    log(INFO, str(cups[num1index+1]))
    log(INFO, str(cups[num1index+2]))
...
def executeMove(cups, currentCupIndex):
    ...
    for index in pickUpIndices:
        if index >= 1000000:
            wrappedIndex = index % 1000000
        indexMod9 = index % 1000000
...
def getCups(lines):
    ...
    # Fill the remaining cups up to one million
