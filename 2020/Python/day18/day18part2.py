...
def puzzleSolution1(lines):
    ...
    log(INFO, "Test 2: Expect 231")
    t1 = evaluate('1 + 2 * 3 + 4 * 5 + 6')
    log(INFO, "Res: " +str(t1))
    
    log(INFO, "Test 3: Expect 46")
    t1 = evaluate('2 * 3 + (4 * 5)')
    log(INFO, "Final solution: " + str(t1))
    
    log(INFO, "Test 3: Expect 1445")
    t1 = evaluate('5 + (8 * 3 + 9 + 3 * 4 * 3)')
    log(INFO, "Final solution: " + str(t1))
    
    log(INFO, "Test 4: Expect 669060")
    ...
    if (len(tokens) == 1):
        ...
        if (not '(' in tokens):
            ...
            if (len(tokens) > 3):
                ...
                else: # + operations need to be solved first
                    # find first + operation
                    log(DEBUG, "Solving + operation")
                    for tokenIndex in range(len(tokens)):
                        token = tokens[tokenIndex]
                        if (token == '+'):
                            plusSignIndex = tokenIndex
                            break
                    numberLeftToPlusSign  = int(tokens[plusSignIndex-1])
                    numberRightToPlusSign = int(tokens[plusSignIndex+1])
                    result = numberLeftToPlusSign + numberRightToPlusSign
                    log(DEBUG, "Reducing " + str(tokens[plusSignIndex-1]) + " + " + str(tokens[plusSignIndex+1]))
                    firstPart = tokens[0:plusSignIndex-1]
                    log(DEBUG, "first : " + str(firstPart))
                    secondPart = tokens[plusSignIndex+2:]
                    log(DEBUG, "second: " + str(secondPart))
                    newList = []
                    for token in firstPart:
                        newList.append(token)
                    newList.append(result)
                    for token in secondPart:
                        newList.append(token)
                    log(DEBUG, "Evaluate:           " + str(newList))
                    return evaluateTokens(newList)
